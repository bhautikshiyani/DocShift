...
const RGBtoCMYK = () => {
  ...
  console.log("🚀 ~ RGBtoHSL ~ rgbtocmyk:", rgbtocmyk)
  const onCopy = (text) => {
    ...
    <div className='grid grid-cols-2'>
      ...
      <div className="text-black dark:text-white">
        <div className='flex items-center gap-3'>
          <strong>{`cmyk(${formatNumber(rgbtocmyk[0])}%, ${formatNumber(rgbtocmyk[1])}%, ${formatNumber(rgbtocmyk[2])}%, ${formatNumber(rgbtocmyk[3])}%)`}</strong>
          <CustomTooltip
            hexColor={`cmyk(${formatNumber(rgbtocmyk[0])}% ${formatNumber(rgbtocmyk[1])}%, ${formatNumber(rgbtocmyk[2])}%, ${formatNumber(rgbtocmyk[3])}%)`}
            onCopy={onCopy}
        ...
        <div className="color-converter__channels">
          <div className="color-converter__channel">Cyan: <strong>{formatNumber(rgbtocmyk[0])} %</strong></div>
          <div className="color-converter__channel">Magenta: <strong>{formatNumber(rgbtocmyk[1])} %</strong></div>
          <div className="color-converter__channel">Yellow: <strong>{formatNumber(rgbtocmyk[2])} %</strong></div>
          <div className="color-converter__channel">Black: <strong>{formatNumber(rgbtocmyk[3])} %</strong></div>
        </div>
      </div>
    </div>
