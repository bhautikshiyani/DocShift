      ...
      <div className='grid grid-cols-2'>
        ...
        <div className="text-black dark:text-white">
          <div className='flex items-center gap-3'>
            <strong>{`hsl(${formatNumber(rgbtohsl[0])}deg ${formatNumber(rgbtohsl[1])}% ${formatNumber(rgbtohsl[2])}%)`}</strong>
            <CustomTooltip
              hexColor={`hsl(${formatNumber(rgbtohsl[0])}deg ${formatNumber(rgbtohsl[1])}% ${formatNumber(rgbtohsl[2])}%)`}
              onCopy={onCopy}
          ...
          <div className="color-converter__channels">
            <div className="color-converter__channel">Hue: <strong>{formatNumber(rgbtohsl[0])} °</strong></div>
            <div className="color-converter__channel">Saturation: <strong>{formatNumber(rgbtohsl[1])} %</strong></div>
            <div className="color-converter__channel">Lightness: <strong>{formatNumber(rgbtohsl[2])} %</strong></div>
          </div>
        </div>
      </div>
