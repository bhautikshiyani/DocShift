            ...
            <div className='grid grid-cols-2'>
                ...
                <div className="text-black dark:text-white">
                    <div className='flex items-center gap-3'>
                        <strong>{`hsl(${formatNumber(labtohsl[0])}deg ${formatNumber(labtohsl[1])}% ${formatNumber(labtohsl[2])}%)`}</strong>
                        <CustomTooltip
                            hexColor={`hsl(${formatNumber(labtohsl[0])}deg ${formatNumber(labtohsl[1])}% ${formatNumber(labtohsl[2])}%)`}
                            onCopy={onCopy}
                    ...
                    <div className="color-converter__channels">
                        <div className="color-converter__channel">Hue: <strong>{formatNumber(labtohsl[0])} °</strong></div>
                        <div className="color-converter__channel">Saturation: <strong>{formatNumber(labtohsl[1])} %</strong></div>
                        <div className="color-converter__channel">Lightness: <strong>{formatNumber(labtohsl[2])} %</strong></div>
                    </div>
                </div>
            </div>
